...
public class B_Plus_Tree<K extends Comparable<? super K>, V> implements Serializable {
	 ...
	 */
	//this shoud beloaded from the properties file 
	//i think it is like the node size 
	 ...
	 */
	// we could just serialize the root node 
	private Node root;
	...
	public B_Plus_Tree(int branchingFactor) {
		...
		while (!queue.isEmpty()) {
			...
			while (!queue.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if (it.hasNext())
						sb.append(", ");
					if (node instanceof B_Plus_Tree.InternalNode)
